# RIPPLE Order & Inquiry Workflow

Customer order and inquiry requests from the website are submitted to Google Workspace through a shared Google Apps Script Web App endpoint. The owner manages orders and inquiries directly in Google Sheets.
...
Customer browser
       ...
       └── Contact page (ContactForm.astro)
           ...
           Google Apps Script Web App (doPost)
                │
                ├── handleOrder() (not used for inquiries)
                │
                └── handleInquiry()
                    ├── Inquiries sheet
                    └── Email notification
...
Only the website needs setup (env vars + Apps Script deployment). Orders and inquiries flow **outbound** from the website to Sheets. No pipeline changes are needed.
...
The contact form submits inquiries through the same Apps Script endpoint used for orders. The Apps Script routes submissions by payload type: `order` payloads go to `handleOrder()`, `inquiry` payloads go to `handleInquiry()`.
...
Contact Form
    │
    ▼
Client-side fetch POST to PUBLIC_SUBMISSION_ENDPOINT
    │
    ▼
Apps Script doPost()
    │
    ▼
Payload contains token + inquiry {}
    │
    ▼
handleInquiry()
    │
    ├── generateInquiryId()
    ├── writeInquiry() → Inquiries sheet
    └── Email notification to owner
```

### Website Environment Variables

The same `PUBLIC_SUBMISSION_ENDPOINT` and `PUBLIC_ORDER_TOKEN` are used for both orders and inquiries. No additional configuration is needed.
...
- `INQ` — constant prefix (Inquiry)
- `YYYYMMDD` — date of submission
- `###` — zero-padded sequence number, resets daily

Generated by `generateInquiryId()` in the Apps Script.
...
```json
{
  "token": "RIPPLE_Order_Submission_a84f92d1",
  "inquiry": {
    "name": "...",
    "email": "...",
    "phone": "...",
    "topic": "general | bakery | sewing | custom | product | feedback | other",
...
The contact form provides these topic options:
...
The contact form implements three states:

| State | Behavior |
|---|---|
| **Loading** | Submit button shows "Sending..." and is disabled |
| **Success** | Form resets, confirmation message "Thanks for reaching out! We'll get back to you soon." is shown |
| **Error** | Red feedback message appears below the form |

If no endpoint is configured (`PUBLIC_SUBMISSION_ENDPOINT` is empty), the form simulates a successful submission after an 800 ms delay — no data is written to Sheets.

### Testing

1. Set `PUBLIC_SUBMISSION_ENDPOINT` and `PUBLIC_ORDER_TOKEN` in `.env`
2. Run `npm run dev`
3. Open `/contact`
4. Fill all fields and submit
5. Verify:
   - "Sending..." appears briefly on the button
   - Confirmation message appears
   - A row appears in the **Inquiries** sheet
   - Email notification arrives at the configured notification address

### Future: Inquiry → Convert to Order

A future workflow could allow the owner to convert an inquiry into an order from within Google Sheets. This would:
